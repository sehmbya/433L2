...
/*******************************************************************************************
* FFTInit()- Initializes real FFT instances
*******************************************************************************************/
void FFTInit(void){
    //Instantiate left and right buffer FFTs
    // func arg 3: 0 --> forward transform
    // func arg 4: 1 --> enable bit reversal of output
    arm_rfft_init_q31(&arm_rfft_instance_q31_left, FFT_LENGTH, 0, 1);
    arm_rfft_init_q31(&arm_rfft_instance_q31_right, FFT_LENGTH, 0, 1);
}
...
\
        ...
        //only computing for the left channel
        arm_absmax_q31(&fftMagResultLeft[0],
                DSP_SAMPLES_PER_BLOCK,
                &fftPeakFrequencyValue,
                &peak_frequency_index);

        //right channel FFT, magnitude
